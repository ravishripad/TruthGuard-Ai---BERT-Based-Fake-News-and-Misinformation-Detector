...
from app.utils.web_scraper import web_scraper
...
@router.post("/url-predict")
@limiter.limit("10/minute")
async def url_predict(
    request: Request,
    body: UrlPredictionRequest,
    current_user: dict = Depends(get_current_user)
):
    """
    Scrape a news article URL, extract the title/body text, and run the
    standard verification pipeline against the scraped content.
    """
    try:
        user_id = str(current_user["_id"])
        logger.info("[url-predict] user=%s | url=%s", user_id, body.url)

        try:
            scraped_article = web_scraper.scrape(body.url)
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc))
        except Exception as exc:
            raise HTTPException(status_code=502, detail=f"Unable to scrape article URL: {str(exc)}")
        ...
        news_validation = news_validator.validate_claim(title)
        logger.info(
            "[url-predict] news_validation status=%s relevant=%d",
            news_validation.get("verification_status", "n/a") if news_validation else "n/a",
            news_validation.get("relevant_articles", 0) if news_validation else 0,
        )
        ...
        if ai_result:
            ...
            logger.info(
                "[url-predict] Gemini answer=%s conf=%.2f context_articles=%d",
                ai_result["prediction"].upper(),
                ai_result["confidence"],
                ai_result.get("context_articles_used", 0),
            )
        else:
            logger.info("[url-predict] Gemini unavailable - falling back to BERT")
            formatted_input = f"{title} [SEP] {text}"
            model, tokenizer, checkpoint = get_model()
            bert_result = predict_fake_news(formatted_input, model, tokenizer, checkpoint)
            bert_result["text"] = title
            final_result = {
                **bert_result,
                ...
                "extracted_from_url": True,
                "reasoning": (
                    f"BERT model classified this as {bert_result['prediction']} with "
                    f"{bert_result['confidence']*100:.1f}% confidence based on the scraped article text."
                ),
            }

        final_result = news_validator.enhance_prediction(final_result, ai_result, news_validation)
        final_result["url_extraction"] = {
            "url": scraped_article["url"],
            "title": title,
            "source": scraped_article.get("source"),
            "domain": scraped_article.get("domain"),
        ...
        prediction_record = {
            ...
            "is_fake": final_result["is_fake"],
            "from_url": True,
            "source_url": scraped_article["url"],
            "scraped_source": scraped_article.get("source"),
            "created_at": datetime.utcnow(),
        }
        await predictions_collection.insert_one(prediction_record)

        logger.info(
            "[url-predict] DONE user=%s | title='%.60s' | result=%s | confidence=%.2f",
            user_id,
            title,
            final_result["prediction"].upper(),
            final_result["confidence"],
        ...
        logger.error("[url-predict] ERROR user=%s | %s", str(current_user.get("_id", "?")), e, exc_info=True)
